    ...
    public class HeadPanels
    {
        ...
        public Dictionary<string, ElementId> GetPanelOfCircuitGroups(Dictionary<string, List<ElectricalSystem>> endCircuitGroups)
        {
            ...
            foreach (KeyValuePair<string, List<ElectricalSystem>> kvpEs in endCircuitGroups)
            {
                ...
                //if (!headPanelOfGroups.ContainsKey(groupStr))
                //    headPanelOfGroups[groupStr] = [];
                ...
                foreach (ElectricalSystem endCircuit in listCircuit)
                {
                    ...
                    while (feederCircuit.Count > 0)
                    {
                        ...
                        foreach (ElectricalSystem es in circuitsAll)
                        {
                            ...
                            //если цепь в нагрузках, то прерываем эту итерацию и переходим к следующей итерации
                            if (circuitsLoads.Contains(esId))
                            {
                                continue;
                            }

                            string valueGroupCircuit = GetValuePramGroup(es, paramDefinition);
                            ...
                            {
                                // значит это питающая цепь — добавляем её в circuitFeeder
                                circuitFeeder.Add(es);
                            }
                        }

                        // если питающей цепи нет, то это и есть головная панель этой группы
